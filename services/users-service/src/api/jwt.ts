...
export type JwtUser = Pick<User, 'username'>;
...
export const buildJwt = (secret: string, _jwt: typeof jwt = jwt) => ({
  getUserFromHeader: async (orig: {
    ...
    const authHeader = orig.originalEvent.headers['authorization'] || orig.originalEvent.headers['Authorization'];
  ...
  verify: (s: string) => _jwt.verify(s, secret)
});
